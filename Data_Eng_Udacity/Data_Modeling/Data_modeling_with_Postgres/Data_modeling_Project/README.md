...
- The data types are structured (we know before-hand the structure of the json we need to analyze, and where and how to extract and transform each field)
...
1. **data** folder nested at the home of the project, where all needed json reside.
...
3. Here we load the file as a data frame using a pandas function called read_json().

4. For each row in the data frame we select the fields we are interested in:
...
8. We load our data as a data frame same way as with songs data. 

9. We select rows where page = 'NextSong' only

10. We convert ts column where we have our start_time as timestamp in milliseconds to datetime format. We obtain the parameters we need from this date (day, hour, week, etc), and insert everything into our time dimensional table.
...
* **Otieno Onyango** - [Github](https://github.com/vicnoo/) - [LinkedIn](https://www.linkedin.com/in/otieno-onyango-48271275/)
